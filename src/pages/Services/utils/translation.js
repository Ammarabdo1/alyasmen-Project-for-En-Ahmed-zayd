export const AR = {
    header: 'خدماتنا',

    outer:'التصاميم الخارجيه',
    outer_des:'التصميم الخارجي هو عملية تصميم الجزء الظاهر الخارجي من المبنى، مثل السقف والواجهات وأي عناصر مرئية خارجية أخرى، فهو يعكس شخصية مالكه وينقل رسالة محددة من خلال إنشاء تصميم مميز وفريد. يحدد التصميم الخارجي مدى انسجام المبنى من الداخل والخارج ومع الجوار المحيط، بالإضافة إلى تأثيره إيجابًا على وظائف المبنى، من خلال التخطيط الجيد للتصميم الخارجي وإنشاء مساحات خارجية عمليّة وجميلة',

    internal:'التصاميم الداخليه',
    internal_des:'تكمن أهمية التصميم الداخلي في استغلال المساحة المتاحة للمكان بأفضل شكل بناءً على الوظيفة واحترافية فريق عملنا في التصميم',

    landscape:'لاند سكيب وتنسيق حدائق',
    landscape_des:'للنباتات دور حيوي في الحديقة فهي مصدر الخضرة، وتساهم في تنقية الجو من الملوثات وتلطيفه وتخفيف الضوضاء، كما أنها توفر الظل وتضبط حرارة الجو وتزيد نسبة الرطوبة، وأيضا فإن مزيج خضرة النباتات مع ألوان الزهور يمنح أثرا نفسيا رائعا، يبعث على السرور والسعادة والراحة النفسية والهدوء، ويخفف عن المواطنين مشكلاتهم الحياتية وهمومهم مما يساعدهم على تجديد النشاط والحيوية، وزيادة القدرة على العطاء والإنتاج. نظرا لذلك اولينا اهتماماً خاص بخدمه تنسيق الحدائق تحت إشراف هندسي إحترافي ومتميز',

    area:'خدمات هندسه المساحه',
    area_des:'الرفع المساحي ينتج عندما يتم الإعداد لعمل القرار المساحي وهو بمثابة الخريطة التي يتم طباعتها على الأرض، ومن شأنها أن توضح كافة جهات وقياسات الأرض سواء الأبعاد أو المساحة الكلية والارتفاع، عند الشروع في بناء أي عقار أيا كان الغرض منه، لتحديد الزوايا المشتركة مع البناء من مباني وأراضٍ مجاورة، ونقدم لك في مكتبنا  إعداد التقرير المساحي لتقديم للجهة المختصة خاليا من العيوب ومناسب لكود البناء المصري بنسبة 100%',

    finish:'أعمال الترميم',
    finish_des:'تتأثر العديد من البنايات بالعوامل الزمنية، حيث تبدأ بعض الشروخ والتصدعات في الظهور مما قد يعرض المبنى للانهيار، وهذا من شأنه الإضرار بسلامة السكان والمباني المجاورة؛ هنا يأتي دورنا الهندسي لمعرفة طرق الإصلاحات اللازم تنفيذها لضمان سلامة المبنى والمحافظة عليه وعلي ساكنيه.',

    build:'الاعمال الإنشائية',
    build_des:'الاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائيةالاعمال الإنشائية',

    look:'الإشراف والتنفيذ',
    look_des:'الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ الإشراف والتنفيذ',
}

export const USA = {
    header: 'services',

    outer: 'Exterior designs',
    outer_des:'Exterior design is the process of designing the visible outer part of a building, such as the roof, facades, and any other external visual elements. It reflects the personality of its owner and conveys a specific message by creating a distinctive and unique design. Exterior design determines the extent to which the building harmonizes from the inside and outside and with the surrounding neighborhood, in addition to its positive impact on the functions of the building, through good planning of the exterior design and the creation of practical and beautiful outdoor spaces',

    internal: 'Interior designs',
    internal_des:'The importance of interior design lies in making the best use of the available space based on the function and the professionalism of our design team',

    landscape: 'Landscape and garden',
    landscape_des:'Plants play a vital role in the garden as they are the source of greenery, contribute to purifying the air from pollutants, softening it and reducing noise, and provide shade, regulate the temperature and increase the humidity. Also, the combination of green plants with the colors of flowers gives a wonderful psychological effect, which inspires joy, happiness, psychological comfort and calm, and relieves citizens of their life problems and worries, which helps them renew their activity and vitality, and increases the ability to give and produce. Therefore, we have paid special attention to the garden coordination service under the supervision of a professional and distinguished engineer',

    area: 'Surveying engineering',
    area_des:'The surveying is produced when the surveying decision is prepared, which is like a map that is printed on the ground, and it is intended to clarify all aspects and measurements of the land, whether dimensions or total area and height, when starting to build any property, whatever its purpose, to determine the common angles with the building of adjacent buildings and lands, and we provide you in our office with the preparation of the surveying report to submit to the competent authority free of defects and 100% suitable for the Egyptian building code',

    finish: 'Restoration works',
    finish_des:'Many buildings are affected by time factors, as some cracks and fissures begin to appear, which may expose the building to collapse, and this may harm the safety of residents and neighboring buildings; here comes our engineering role to know the methods of repairs that must be implemented to ensure the safety of the building and preserve it and its residents',

    build: 'Construction works',
    build_des:'Construction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction worksConstruction works',

    look: 'Supervision',
    look_des:'Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision Supervision ',
}